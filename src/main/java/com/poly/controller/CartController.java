...
public class CartController {
	...
	@PostMapping("/GioHang")
	public String cart(@RequestParam("id") String product_id,@RequestParam("color") String product_color,@RequestParam("quantity") String product_quantity, Model model) {
		...
		if(sessionService.get("Order") == null) {
			...
			for (Orderdetails x : listodd) {
			    if (x.getProductID().getProductId().equals(pd.getProductId()) && x.getColor().equals(product_color)) {
...
public String clear(Model model) {
	...
	public String Pay(@RequestParam("totalmoney") String totalmoney, Model model) {
		...
		if(listodd != null && !listodd.isEmpty()) {
			...
			String dateString = "2023-06-11";
			Date date = Date.valueOf(dateString);
			OD.setUser(userDAO.finduserById("ID001"));
			...
			List<Orders> listOD = ODDAO.findAll();
